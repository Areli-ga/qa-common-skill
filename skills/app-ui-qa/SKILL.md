...
10. Always publish the verified `report.html` to QA Web after every completed run, even when the user did not mention report generation or upload. Report generation, upload, list-row verification, and opening the uploaded report are mandatory finalization steps. Use the QA member's own signed-in Chrome session at `https://qa-platform.giggletools.com/ui/reports`; do not export or reuse browser cookies, Cloudflare Access tokens, or another person's identity. Follow `references/reporting.md` for the authenticated upload and verification path.
...
- For main-smoke S09, wait for《正常故事书》to finish without manual page turns, capture the settlement page, then use a fresh screenshot to locate and tap `下一步`; only that action should return to the main path.
- After every run, upload only the final verified standalone `report.html`, never the evidence directory or a report with local/external image dependencies. Upload is mandatory even when the user's task did not mention it. Use the current QA member's own Cloudflare Access session in signed-in Chrome so the platform records the correct uploader. Never copy browser cookies, access JWTs, or passwords into shell variables, source files, reports, or prompts. Do not declare the run fully finalized until the new report row and uploaded report rendering have been verified; if authentication or QA Web is unavailable, preserve the local report and state that upload finalization is blocked.
...
- Treat an observed App crash as diagnostic evidence, not a transient UI block. Immediately record the case id, device/build, local timestamp, pre-crash action, visible crash state, and relaunch result.
- On iOS, if the system asks whether to share crash information with the developer, always choose the share option and capture the prompt plus the selected/post-share state. This crash-diagnostic sharing is required for developer-side log correlation.
- On Android, collect crash diagnostics immediately from the same `ANDROID_SERIAL` with the crash Logcat buffer, a bounded main/system/crash context window, and `dumpsys activity exit-info` when supported. Keep the full raw files under `logs/`; create a reviewed, secret-redacted text copy for report embedding.
- Embed report-safe crash logs in the standalone HTML with the renderer's `<qa-log src="logs/..." title="..."></qa-log>` directive. The full referenced text is placed in a collapsed log block, so the uploaded HTML remains self-contained. Never embed raw logs before removing passwords, tokens, complete child ids, IPs, audio URLs, and other secrets.
...
- `scripts/render-manual-report.mjs`: render screenshot-backed `report.md` into a self-contained `report.html`; local screenshots are WebP-compressed and embedded as data URIs when conversion is available, and local plain-text logs referenced by `<qa-log>` are embedded as collapsed full-text blocks.
