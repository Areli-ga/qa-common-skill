...
| Skill | 用途 |
| --- | --- |
| `app-ui-qa` | 复合技术栈移动 App 的双端 UI 冒烟与新功能探索性测试。支持主流程、Android 渠道包，以及从 XMind、Excel、图片或鉴权网址转化的新功能执行文档；Android 使用 adb-only，iOS 使用 iPhone Mirroring + PyAutoGUI；输出内联截图与脱敏崩溃日志的单文件 HTML，并在每轮结束后使用 QA 成员自己的登录态上传 QA Web。 |
...
- `app-ui-qa` 每轮结束后必须生成并上传单文件 HTML 报告；iOS 崩溃信息选择共享给开发者，Android 崩溃日志脱敏后通过 `<qa-log>` 内嵌报告。
